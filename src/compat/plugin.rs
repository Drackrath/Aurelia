//! Shared download manager for plugins.
...
pub struct InstalledPlugin {
    /// Installed tag; `"custom"` when external.
    pub version: String,
    /// Directory holding the marker file.
    pub root: PathBuf,
    /// The executable Aurelia invokes.
    pub entry: PathBuf,
}

/// Per-plugin download manager parameters.
pub(crate) struct PluginSpec {
    ...
    /// Whether `path` is an install root.
...
/// The plugin's extraction directory.
...
/// Install root under `base`, if any.
...
async fn latest_release(spec: &PluginSpec) -> Result<PluginRelease> {
    // GitHub lookups reuse token-aware client.
    ...
    // Gitea/GitHub share this JSON shape.
...
/// Download and extract; returns root.
...
/// Remove payload; `false` if absent.
