//! Steam Storefront / SteamSpy lookups for human-facing game metadata
//! (description, categories, genres, user tags, DLC). This is public store data
//! and does not require an authenticated Steam connection.
...
/// If `app_id` is a DLC, return the base ("full game") app id it belongs to.
/// Returns `None` for base games or on any lookup failure.
pub async fn fetch_dlc_parent(client: &reqwest::Client, app_id: u32) -> Option<u32> {
    let url =
        format!("https://store.steampowered.com/api/appdetails?appids={app_id}&l=english&cc=us");
    let resp = client.get(&url).send().await.ok()?;
    let value: serde_json::Value = resp.json().await.ok()?;
    let data = value.get(app_id.to_string())?.get("data")?;

    if data.get("type").and_then(|t| t.as_str()) != Some("dlc") {
        return None;
    }
    // `fullgame.appid` is reported as a string.
    data.get("fullgame")?
        .get("appid")?
        .as_str()
        .and_then(|s| s.trim().parse::<u32>().ok())
        .filter(|&base| base != 0 && base != app_id)
}

/// Fetch just the display name for an app (used to resolve DLC names).
pub async fn fetch_app_name(client: &reqwest::Client, app_id: u32) -> Option<String> {
    fetch_app_details(client, app_id)
        .await
        .ok()
        .flatten()
        .map(|d| d.name)
