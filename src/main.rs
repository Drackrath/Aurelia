...
async fn run(cli: Cli) -> Result<()> {
    ...
    match cli.command {
        ...
        Command::Info {
            ...
            lang,
            country,
        } => cmd_info(app_ids, extended, no_cache, lang, country, json).await,
        Command::Price {
            app_id,
            compare,
            country,
            lang,
        } => cmd_price(app_id, compare, country, lang, json).await,
        Command::Search {
            term,
            count,
            country,
            lang,
        } => cmd_search(term, count, country, lang, json).await,
        Command::Deals {
            scope,
            min_discount,
            count,
            ...
            country,
            lang,
        } => cmd_deals(scope, min_discount, count, start, country, lang, json).await,
        Command::Similar {
            app_id,
            count,
            country,
        ...
        Command::News {
            ...
            count,
            max_length,
        } => cmd_news(app_id, count, max_length, json).await,
        Command::Reviews {
            app_id,
            filter,
            review_type,
            ...
            cursor,
            lang,
        } => cmd_reviews(app_id, filter, review_type, purchase, count, cursor, lang, json).await,
        ...
        Command::Image {
            ...
            if list {
                cmd_image_list(app_id, probe, json).await
            } else {
        ...
        Command::Config { command } => match command {
            ...
            ConfigCommand::Country { country } => cmd_config_country(country, json).await,
